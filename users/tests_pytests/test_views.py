...
def demo_token(client):
    response = client.put(reverse('demo'))
...
class TestDemoUser:
    ...
    def test_new_demo_users_token_created(self, client):
        response = client.put(reverse('demo'))
        demo_token = Token(response.data['token'])
    ...
    def test_demo_user_retrieved_from_cookie(self, client, demo_token):
        response2 = client.put(reverse('demo'))
    ...
    def test_two_different_demo_users_when_no_cookie(self, demo_token):
        client2 = APIClient()
        response2 = client2.put(reverse('demo'))
        demo_token2 = Token(response2.data['token'])
        assert demo_token != demo_token2
